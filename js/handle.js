...
bckgrnd.loop = true;
var toggle = true;
var clickAudio1 = new Audio("./res/click.mp3");
var clickAudio2 = new Audio("./res/click.mp3");
...
function handleVisibilityChange() {
    if (document.hidden) {
        // The user has navigated away from the page (switched tab, minimized browser)
        // or locked their mobile device.
        console.log("Page is hidden. Pausing background music.");
        if (bckgrnd && !bckgrnd.paused) { // Check if audio object exists and is currently playing
            bckgrnd.pause();
        }
    } else {
        // The user has returned to the page.
        console.log("Page is visible. Attempting to resume background music.");
        // Only attempt to play if it was previously playing (or if you always want it to resume)
        // Note: You might need to track a 'wasPlaying' flag if you only want to resume
        // if it was active before hiding. For a simple loop, just playing is often fine.
        playLoop();
    }
}
...
function playLoop() {
    ...
    //alert("sound : " + sound + " glasses : " + glasses);
    if (sound === "true") {
...
//    storePreference("silentZone", "true");
    ...
    mbdiv.innerHTML = "Touch the Element and drag..";
...
function moveSuccess() {
    ...
    //    setTimeout(hideNextPanel, 400);
        ...
        elementArray[0].posinArray = 0;
        var mbdiv = document.getElementById("mbdiv");
        mbdiv.innerHTML = "Quick tap the Element to rotate..";
    //    initialDraw();
        initialDraw();
        startTapAnimation();
...
function startFlip() {
    ...
    //    setTimeout(hideNextPanel, 400);
        ...
        var mbdiv = document.getElementById("mbdiv");
        mbdiv.innerHTML = "Tap and hold the Element to flip.";
        initialDraw();
    //    initialDraw();
        startHoldAnimation();
...
{
    runAnim = true;
    var holdImage1 = new Image();
    holdImage1.src = './img/hold1.png';
    var holdImage2 = new Image();
    holdImage2.src = './img/hold2.png';
	setTimeout(function (){holdAnimFrame(dctx, holdImage1, holdImage2, 0)},0);
}

function holdAnimFrame(context, holdImage1, holdImage2, animcount)
{
	clear(context);
    ...
    if (animcount % 2 === 0) 
	{
        context.drawImage(holdImage1, animElement.x, animElement.y, 2*calib, 5*calib);
    }
	else
	{
		context.drawImage(holdImage2, animElement.x, animElement.y, 2*calib, 5*calib);
	}
    if (runAnim) {
        setTimeout(function () {
            holdAnimFrame(context, holdImage1, holdImage2, animcount)
        }, 250);
    }
    else {
        clearInterval(animref);
	...
	if(animcount % 2 === 0)
	{
		context.drawImage(holdImage1, elementArray[0].x, elementArray[0].y, 2*calib, 5*calib);
	}
    else
    {
        context.drawImage(holdImage2, elementArray[0].x, elementArray[0].y, 2*calib, 5*calib);
    }
	if(runAnim)
	{
		setTimeout(function (){holdAnimFrame(context, holdImage1, holdImage2, animcount)},300);
	}
    else
    {
        clearInterval(animref);
...
function startTapAnimation()
{
	...
	setTimeout(function (){tapAnimFrame(dctx, handImage, 0)},0);
}

function tapAnimFrame(context, handImage, animcount)
{
	clear(context);
    if (glasses != "ingameplay")
        return;
    ...
    elementDraw(dctx, animElement);
    if (animcount % 4 === 0) {
        context.drawImage(handImage, animElement.x, animElement.y, 2*calib, 5*calib);
    }
    if (runAnim) {
        setTimeout(function () {
            tapAnimFrame(context, handImage, animcount)
        }, 250);
    }
    else {
        clearInterval(animref);
	...
	if(animcount % 4 === 0)
	{
		context.drawImage(handImage, elementArray[0].x, elementArray[0].y+calib/2, 2*calib, 5*calib);
	}
	if(runAnim)
	{
		setTimeout(function (){tapAnimFrame(context, handImage, animcount)},250);
	}
    else
    {
        clearInterval(animref);
...
function startHandAnimation()
{
    ...
    var handImage = new Image();
    handImage.src = './img/hand.png';
	setTimeout(function (){handAnimFrame(dctx, handImage, 0)},0);
}
...
//    context.drawImage(image, WIDTH / 2 + calib / 2 + increment, HEIGHT / 2 + calib / 2, calib / 2, calib / 2);
    ...
    elementDraw(dctx, animElement);
    context.drawImage(image, animElement.x - increment, animElement.y + calib/2 + increment, 2*calib, 5*calib);
...
//		clear(dctx);
        ...
        tcanvas.height = dcanvas.height = canvas.height = HEIGHT = window.innerHeight;
...
function nextGame() {
    if (events) {
        ...
        var pro = getPreference("pro");
        if (pro === "false" && gameNumber> easyStage) {
            setTimeout(promptPurchase, 400);
            return;
		}
        else if(gameNumber > hardStage)
        {
            hideCongoDialog();
            events = true;
            backHandler();
		...
		if (fromTutorial == true) {
            if (gameNumber == 0) {
				...
				showMessageBoard();
				var mbdiv = document.getElementById("mbdiv");
				mbdiv.innerHTML = "Tap on the Element to rotate..";
                ref = setInterval(rotationChecker, 300);
                startTapAnimation();
            }
            if (gameNumber == 1) {
...
//                saveSkipTutorial(skipTutorial);
                showMessageBoard();
				var mbdiv = document.getElementById("mbdiv");
				mbdiv.innerHTML = "Let's start with a simple game..";
                hideBannerBox();
            }*/
            if (gameNumber == 2) {
				skipTutorial = "true";
                showMessageBoard();
				var mbdiv = document.getElementById("mbdiv");
				mbdiv.innerHTML = "You are ready now";
                hideBannerBox();
                fromTutorial = false;
                //glasses = "";
                setTimeout(function () {
...
function select(x, y) {
    currentx = currenty = firstx = firsty = undefined;
    startTime = Date.now();
//    moveCount = 0;
    isMoved = false;
    ...
    if (topElement != false) {
        currentx = firstx = x;
        currenty = firsty = y;
        console.log("select called " + firstx + ":" + firsty + " current : " + currentx + ":" + currenty);
...
function onTapHold() {
    // alert("onTapHold called, isMoved : "+isMoved);
    var distance = Math.sqrt(Math.pow(currentx - firstx, 2) + Math.pow(currenty - firsty, 2));
    console.log("onTapHold called " + firstx + ":" + firsty + " current : " + currentx + ":" + currenty + " distance : " + distance);
    if (topElement != false && distance < calib/10) {
...
function deselect(e) {
    ...
    clearTimeout(ontapTimer);
    endTime = Date.now();
    // console.log((endTime-startTime)+" moveCount : "+moveCount);
    // it is a click
