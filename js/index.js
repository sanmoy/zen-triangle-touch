...
//        return;
    } else if (glasses === "wait") {
//        glasses = lastGlass;
        hideWaitDialog();
        //glasses = lastGlass;
//        return;
    } else if (glasses === "hintpurchase") {
...
//        showLoadingGif();
        ...
        {
            gameNumber = lastPlayedGame;
            storePreference("gameNumber", lastPlayedGame);
        }
...
function showAddToHomeScreenButton() {
    var isMobileOS = /iPhone|iPad|iPod/i.test(navigator.userAgent);
    // alert(navigator.userAgent);
    var standAlone = window.matchMedia('(display-mode: standalone)').matches;
    // var isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
    if (isMobileOS && !standAlone) {
        var addToHomeContainer = document.getElementById('addToHomeContainer');
        if (addToHomeContainer) {
            addToHomeContainer.style.visibility = "visible";
            // banner_box.style.opacity="1";
        } else {
            var addToHomeContainer = document.createElement("div");
            addToHomeContainer.setAttribute("id", "addToHomeContainer");
            addToHomeContainer.style.width = Math.round(3 * calib) + "px";
            addToHomeContainer.style.height = Math.round(1 * calib) + "px";
            addToHomeContainer.style.zIndex = "5";
            addToHomeContainer.style.right = "2%";
            addToHomeContainer.style.bottom = "4%";
            addToHomeContainer.style.position = "absolute";

            var addHomeButton = document.createElement("div");
            addHomeButton.setAttribute("class", "img");
            addHomeButton.setAttribute("id", "addHomeButton");
            addHomeButton.style.width = Math.round(3 * calib) + "px";
            addHomeButton.style.height = Math.round(1 * calib) + "px";
            //        installButton.style.left = "2%";
            //        installButton.style.top = "1.3%";
            //        installButton.style.right = "initial";
            addHomeButton.style.bottom = "0";
            addHomeButton.style.cursor = 'pointer';
            addHomeButton.style.backgroundImage = "url('./img/add-to-home.webp')";
            //        installButton.style.zIndex = "5";
            addToHomeContainer.appendChild(addHomeButton);
            document.body.insertBefore(addToHomeContainer, document.body.firstChild);
            addToHomeContainer.style.visibility = "visible";

            addEvent("addHomeButton", showAddToHomeScreen);
        }
    }
}

function showAddToHomeScreen() {
    // This function is called to show the "Add to Home Screen" prompt
